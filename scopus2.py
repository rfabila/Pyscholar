...
def search_affiliation_by_id(list_scopus_id_affiliation):
    ...
    :param list_scopus_id_affiliation: If you are looking for an affiliation,
    you can send the id as a string but if you want to multiple affiliations
    you can send a list of their ids.
    :type list_scopus_id_affiliation: String or List
    :returns: Dictionary where the key is the ID of the affiliation
    and the value associated with the key is a dictionary with
    the following keys: date_created, preferred_name, author_count and document_count.
    :rtype: Dictionary
...
def author_info(author_id, strict=False):
    ...
    #workaround for the way in which scopus is now sending info!
    #It may not work in the future.
    #Update it seems changes are coming to the scopus database.
...
def get_title_abstract_by_idpaper(id_paper=""):
    """
    This function returns a tuple with the identification,
    title and summary of the paper that you searched.
